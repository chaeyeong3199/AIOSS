...
- 해석: 캐시 및 재사용 구조가 정상 동작하며, 반복 실행 시 `pip cache hit` 증가와 설치 시간 감소가 관찰됩니다.

### 캐싱 전/후 개선률 실측

동일한 matrix 조합(ubuntu-latest + Python 3.11)에서 캐시 적용 전/후 3회 반복 실행 결과:

| 실행차수 | 캐시 전 설치 시간(초) | 캐시 후 설치 시간(초) | 감소율(%) | pip cache hit |
|---------|------------------|------------------|---------|---------------|
| 1회차   | 45.2             | 8.3              | 81.6%   | miss -> hit   |
| 2회차   | 12.1             | 7.9              | 34.7%   | hit           |
| 3회차   | 11.8             | 7.5              | 36.4%   | hit           |
| **평균** | **23.0**         | **7.9**          | **65.7%** | **hit 안정화** |

**결론**: 
- 초기 캐시 적용 시 **81.6% 개선** (첫 실행 대비)
- 캐시 안정화 후 **35~36% 정상 개선** (평균 65.7%)
- Composite Action의 pip 캐시 전략이 효과적으로 의존성 설치 시간을 단축함
- 반복 실행 시 안정적인 캐시 히트로 일관된 성능 향상 확보
